...
const JobCardUI = ({
  ...
  const handleBookmarkClick = async () => {
    ...
    try {
      await onSaveToggle(job.id, bookmarked);
    } catch (error) {
      setBookmarked(!bookmarked);
  ...
  return (
    ...
    >
      {/* Applied Sticker Badge - Top Left */}
      {isApplied && (
        <Box
          style={{
            position: 'absolute',
...
const SavedJobs = (): JSX.Element => {
  ...
  const savedJobs = myJobs.filter((job) => job.isJobSaved);
  ...
  if (savedJobs.length === 0) {
    return (
      <Paper p={60} radius="md" withBorder>
        <Center>
          <Text c="dimmed">No saved jobs yet</Text>
        </Center>
      </Paper>
    );
  }
